...
class ZmitiContentApp extends Component {
	...
	render() {
		...
		switch (this.props.theme) {
			...
			<div onTouchTap={this.doAgin.bind(this,this.props.gk<window.gkConfig.length)} className='zmiti-dangjian-result-btn'>
			<span><img src='./assets/images/refresh.png'/></span>
			<span>{this.props.gk<window.gkConfig.length? '继续答题':'再做一次' }</span>
			</div>
	...
	chooseMyAnswer(i) {
		...
		if (!this.props.myAnswer[this.state.currentQid] && this.props.myAnswer[this.state.currentQid] !== 0
			...
			if (this.props.questionType === 'single') { //单选题目
				...
				this.state.iNow = i;
				this.forceUpdate();
				if(this.props.question[this.state.currentQid].answer[i].isRight){
					this.rightAnswerCount++;
				}

				if (this.props.question[this.state.currentQid + 1] && window.gkConfig[this.props.gk] !== this.state.currentQid+1) {
					setTimeout(() => {
						this.doNext();
					}, 500)
				}else{
